<template>
  <div class="topbar">
    ...
    <div class="row">
      ...
      <button class="primary" @click="save" :disabled="saving">{{ saving ? 'Saving…' : 'Save overrides' }}</button>
    </div>
  </div>
    ...
    <table>
      ...
      <tbody>
        <tr v-for="m in modules" :key="m.key">
          ...
          <td><span class="badge" :class="data.role_defaults[m.key] ? 'active' : 'offline'">{{ data.role_defaults[m.key] ? 'allowed' : 'denied' }}</span></td>
          <td>
          ...
          <td><span class="badge" :class="effective(m.key) ? 'active' : 'offline'">{{ effective(m.key) ? 'yes' : 'no' }}</span></td>
        </tr>
      </tbody>
    </table>
  </div>
